...
def conversation():
    ...
    action = ["Awesome!", "I had a fun weekend",
              "I went swimming in our community pool", "What did you do over the weekend?"]
    critic = "Try again"
    affirmation = "That sounds good"
    reinforcement = "Awesome job!"

    food = ["Awesome!", "Man, these cookies are delicious",
    ...
    while convoDone == False:
        ...
        girlspeak = client.synthesize_speech(
            texttospeech.types.SynthesisInput(text=name), girl, audio_config)
        counter += 1
        speak(girlspeak, counter)
        notYet = True
        if convo == action:
            for item in convo:
                girlspeak = client.synthesize_speech(
                    texttospeech.types.SynthesisInput(text=item), girl, audio_config)
                counter += 1
                speak(girlspeak, counter)
                # if len(item.split(" ")) != 1:
                ...
                if convo.index(item) == len(convo)-1:
                    ...
                    while notYet:
                        if response == None:
                            coachspeak = client.synthesize_speech(texttospeech.types.SynthesisInput(
                                text=("That's not an action")), coach, audio_config)
                            counter += 1
                            speak(coachspeak, counter)
                            time.sleep(1.5)
                            coachspeak = client.synthesize_speech(
                                texttospeech.types.SynthesisInput(text=critic), coach, audio_config)
                            counter += 1
                            speak(coachspeak, counter)
                            time.sleep(1.5)
                            girlspeak = client.synthesize_speech(
                                texttospeech.types.SynthesisInput(text=convo[-1]), girl, audio_config)
                            counter += 1
                            speak(girlspeak, counter)
                            time.sleep(1.5)
                            response = transcribe_file(record_file(5))
                            ...
                            if verify_response(twenty(i), "action") and ("I" not in response and "did" not in response):
                                coachspeak = client.synthesize_speech(
                                    texttospeech.types.SynthesisInput(text=affirmation), coach, audio_config)
                                counter += 1
                                speak(coachspeak, counter)
                                time.sleep(1.5)
                                coachspeak = client.synthesize_speech(texttospeech.types.SynthesisInput(
                                    text="Let's give a better response"), coach, audio_config)
                                counter += 1
                                speak(coachspeak, counter)
                                time.sleep(1.5)
                                coachspeak = client.synthesize_speech(texttospeech.types.SynthesisInput(
                                    text=("Say I did" + str(i))), coach, audio_config)
                                counter += 1
                                speak(coachspeak, counter)
                                time.sleep(1.5)
                                response = transcribe_file(record_file(5))
                                ...
                                if "I" in response or "did" in response:
                                    coachspeak = client.synthesize_speech(
                                        texttospeech.types.SynthesisInput(text=reinforcement), coach, audio_config)
                                    counter += 1
                                    speak(coachspeak, counter)
                                    time.sleep(1.5)
                                    girlspeak = client.synthesize_speech(
        ...
        speak(girlspeak, counter)
        time.sleep(5)
        remove("/home/prince/Desktop/HACKIIITD/hawking-native/GUI/conversation")
        convoDone = True
